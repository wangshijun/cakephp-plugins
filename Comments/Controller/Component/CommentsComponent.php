 ...
 * Customizable in beforeFilter()
 *
 * @var string Name of the username field
 */
		...
		if (!empty($this->Controller->data)) {
			...
			if (!is_null($result)) {
				if ($result) {
					...
					$this->flash(__d('comments', 'The Comment has been saved.'), 'success');
					...
					$this->flash(__d('comments', 'The Comment could not be saved. Please, try again.'), 'error');
				}
			}
		} else {
 ...
 */
	public function callback_toggleApprove($modelId, $commentId) {
		...
		if ($this->Controller->{$this->modelName}->commentToggleApprove($commentId)) {
			$this->flash(__d('comments', 'The Comment status has been updated.'), 'success');
		} else {
			$this->flash(__d('comments', 'Error appear during comment status update. Try later.'), 'error');
		}
	}
 ...
 */
	public function callback_delete($modelId, $commentId) {
		if ($this->Controller->{$this->modelName}->commentDelete($commentId)) {
			$this->flash(__d('comments', 'The Comment has been deleted.'), 'success');
		} else {
			$this->flash(__d('comments', 'Error appear during comment deleting. Try later.'), 'error');
		}
		$this->redirect();
	}
